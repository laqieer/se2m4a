...
for audio_file in sys.argv[1:]:
    ...
    for lookahead in lookaheads:
        ...
        t = end - start
        time_audio.append(t)
        print("lookahead=%d: SNR: %.2fdB Time:%.2fs" % (lookahead, SNR, t))
...
axis[1].set_title('Time-Lookahead')
axis[1].set_xlabel('lookahead')
axis[1].set_ylabel('compression time/s')
axis[1].set_xticks(lookaheads)
for time in time_all:
    axis[1].plot(lookaheads, time)
